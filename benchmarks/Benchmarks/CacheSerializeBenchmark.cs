...
{
    private readonly KeyValueCache _cache = new(typeof(TestSerial));
    private readonly KeyValueConfiguration _configuration = new();
    private readonly CancellationToken _cancellationToken = new();
    private readonly TestSerial _testSerial = new()
    {
        String = "TestString",
    ...
    private readonly byte[] _testSerialBytes;

    public CacheSerializeBenchmark()
    {
        var newLine = (char)_configuration.ValueEnd + Encoding.UTF8.GetString(_configuration.NewLine);
        var keyValueSeparator = Encoding.UTF8.GetString(new[] { _configuration.Space, _configuration.ValueStart, _configuration.Space });
        var stringSeparator = (char)_configuration.StringSeparator;
        var arrayStart = (char)_configuration.ArrayStart;
        var arraySeparator = Encoding.UTF8.GetString(new[] { _configuration.ArraySeparator, _configuration.Space });
        var arrayEnd = (char)_configuration.ArrayEnd;
        var stringEscape = Encoding.UTF8.GetString(new[] { _configuration.StringIgnoreCharacter, _configuration.StringSeparator });
        
        var deserialTestSerial = 
        ...
        _testSerialBytes = Encoding.UTF8.GetBytes(deserialTestSerial);
    }

    [Benchmark]
    public async ValueTask<TestSerial> CacheDeserializeAsync()
    {
        using var memoryStream = new MemoryStream(_testSerialBytes);
        return await Deserializer.DeserializeStreamAsync<TestSerial>(memoryStream, _cache, _configuration,
            _cancellationToken);
    }
    ...
    public void CacheSerialize()
    {
        ...
        Serializer.Serialize(_testSerial, memoryStream, _cache, _configuration);
    }
}
