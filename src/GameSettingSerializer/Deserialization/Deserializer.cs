...
internal static class Deserializer
{
		...
		KeyValueConfiguration config)
	{
		...
		ValueReader.SetProperty(buildObject, property, trimmedValue, config);
		return true;
	}
		...
		KeyValueConfiguration config)
	{
		...
		ValueReader.SetProperty(buildObject, property, stringBytes, config);
		return true;
	}
	...
	private static bool TryAssignArray(ref SequenceReader<byte> reader, object buildObject, KeyValueProperty property,
		KeyValueConfiguration config)
	{
		var arrayEndIndex = ValueReader.IndexOfArrayEnd(reader.UnreadSpan, config, out var arraySize);
		if (arrayEndIndex == -1)
		{
			return false;
		}
		...
		ValueReader.SetArrayProperty(buildObject, property, remainingArray, config, arraySize);
	...
	{
		FileType.String => ValueParser.ParseString(propertyValue),
		FileType.Boolean => ValueParser.ParseBool(propertyValue),
		FileType.DateTime => ValueParser.ParseDateTime(propertyValue, config.DateTimeFormat),
		FileType.DateTimeOffset => ValueParser.ParseDateTimeOffset(propertyValue, config.DateTimeFormat),
		FileType.TimeSpan => ValueParser.ParseTimeSpan(propertyValue),
		FileType.Guid => ValueParser.ParseGuid(propertyValue),
		FileType.Int8 => ValueParser.ParseInt8(propertyValue),
		FileType.UInt8 => ValueParser.ParseUInt8(propertyValue),
		FileType.Int16 => ValueParser.ParseInt16(propertyValue),
		FileType.UInt16 => ValueParser.ParseUInt16(propertyValue),
		FileType.Int32 => ValueParser.ParseInt32(propertyValue),
		FileType.UInt32 => ValueParser.ParseUInt32(propertyValue),
		FileType.Int64 => ValueParser.ParseInt64(propertyValue),
		FileType.UInt64 => ValueParser.ParseUInt64(propertyValue),
		FileType.Float32 => ValueParser.ParseFloat32(propertyValue),
		FileType.Float64 => ValueParser.ParseFloat64(propertyValue),
		FileType.Float128 => ValueParser.ParseFloat128(propertyValue),
